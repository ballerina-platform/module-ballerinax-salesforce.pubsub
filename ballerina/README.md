...
The `ballerinax/salesforce.pubsub` package provides a `Publisher` client and a `Listener` service for the Pub/Sub API. A `Publisher` publishes events to one topic with per-event results and batch chunking. A `Listener` subscribes to one or more topics with sequential, checkpointed, at-least-once delivery per topic, automatic replay checkpointing, and normalized Change Data Capture bodies. Generated gRPC records, OAuth token lifecycle, and Avro encoding/decoding stay internal to the package.
